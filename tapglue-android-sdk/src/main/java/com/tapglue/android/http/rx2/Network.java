...
import com.tapglue.android.http.EventFeedToListRx2;
...
import com.tapglue.android.internal.rx2.SessionStore;
import com.tapglue.android.internal.rx2.UUIDStore;
import com.tapglue.android.rx2.RxPage;
...
public class Network {
    ...
    public Network(ServiceFactory serviceFactory, Context context) throws Exception {
        ...
        uuidStore.get().doOnNext(new UUIDConsumer()).subscribe();
    ...
    public Observable<Void> logout() {
        return service.logout().doOnComplete(sessionStore.clear());
    }
    ...
    public Observable<Void> deleteCurrentUser() {
        return service.deleteCurrentUser().doOnComplete(sessionStore.clear());
    }
    ...
    public void clearLocalSessionToken() throws Exception {
        sessionStore.clear().run();
    }
    ...
    public Observable<List<Event>> retrieveEventFeed() {
        return service.retrieveEventFeed().map(new EventFeedToListRx2());
    }
    ...
    private class SessionTokenExtractor implements Function<User, User> {
    ...
    private class UsersExtractor implements Function<UsersFeed, List<User>> {
        @Override
        public List<User> apply(UsersFeed feed) {
    ...
    private class UUIDConsumer implements Consumer<String> {
    ...
    private static class RxPageCreator<T> implements Function<FlattenableFeed<T>, RxPage<T>> {
        ...
        @Override
        public RxPage<T> apply(FlattenableFeed<T> feed) {
